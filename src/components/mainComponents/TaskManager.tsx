...
const TaskManager = () => {
  ...
  const [taskType, setTaskType] = useState<"Essay" | "Test" | "Discussion">("Essay");
  const [essayQuestion, setEssayQuestion] = useState("");
  const [discussionPrompt, setDiscussionPrompt] = useState("");
  const [tag, setTag] = useState("");
  const [testIntro, setTestIntro] = useState(""); // ➕ Вступна стаття до тесту
    ...
    const payload = {
      ...
      content:
        taskType === "Essay"
          ...
          : {
              intro: testIntro, // ➕ Додаємо вступ до тесту
  ...
  return (
    ...
    >
      ...
      {taskType === "Discussion" && (
        <TextField
          label="Стаття із запитанням для обговорення"
          fullWidth
          multiline
      ...
      {taskType === "Test" && (
        <Box display="flex" flexDirection="column" gap={2}>
          <TextField
            label="Вступна стаття до тесту"
            fullWidth
            multiline
            minRows={3}
            value={testIntro}
            onChange={(e) => setTestIntro(e.target.value)}
            sx={{ mb: 2 }}
          />
